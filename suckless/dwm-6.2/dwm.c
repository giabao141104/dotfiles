...
struct Monitor {
	...
	int wx, wy, ww, wh;   /* window area  */
        int gappih;           /* horizontal gap between windows */ /* vanitygaps */
...
static int drawstatusbar(Monitor *m, int bh, char* text); /*status2d*/
...
/*static void tile(Monitor *);*/ /* vanitygaps */
...
static char stext[1024]; /*status2d*/
static int screen;
...
static xcb_connection_t *xcon; /*swallow*/
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->instance || strstr(instance, r->instance)))
		{
			...
			c->isfloating = r->isfloating;
                        ...
                        c->isterminal = r->isterminal; /*swallow*/
                        c->noswallow  = r->noswallow; /*swallow*/
			c->tags |= r->tags;
...
cleanup(void)
{
	...
	for (i = 0; i < CurLast; i++)
        ...
        for (i = 0; i < LENGTH(colors) + 1; i++) /*status2d*/
		free(scheme[i]);
...
destroynotify(XEvent *e)
{
	...
	if ((c = wintoclient(ev->window)))
		unmanage(c, 1);
        else if ((c = swallowingclient(ev->window))) /*swallow*/
                unmanage(c->swallowing, 1); /*swallow*/
}
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
                sw = m->ww - drawstatusbar(m, bh, stext); /*status2d*/
	}
...
{
        Client *c, *t = NULL, *term = NULL; /*swallow*/
	Window trans = None;
	XWindowChanges wc;

	c = ecalloc(1, sizeof(Client));
	c->win = w;
        c->pid = winpid(w); /*swallow*/
	/* geometry */
	c->x = c->oldx = wa->x;
	c->y = c->oldy = wa->y;
	c->w = c->oldw = wa->width;
...
setup(void)
{
	...
	/* init appearance */
        scheme = ecalloc(LENGTH(colors) + 1, sizeof(Clr *)); /*status2d*/
        scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], 3); /*status2d*/
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
        ...
        if (!(xcon = XGetXCBConnection(dpy))) /*swallow*/
                die("dwm: cannot get xcb connection\n"); /*swallow*/
	checkotherwm();
	setup();
#ifdef __OpenBSD__
	if (pledge("stdio rpath proc exec ps", NULL) == -1) /*swallow*/
		die("pledge");
#endif /* __OpenBSD__ */
	scan();
	run();
