...
#define SETPROP(p, q) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             "prop=\"`xprop -id $2 $0 " \
             "| sed \"s/^$0(STRING) = \\(\\\\\"\\?\\)\\(.*\\)\\1$/\\2/\" " \
             "| xargs -0 printf %b | dmenu -bw 0 -i -p \"URL bar\"`\" &&" \
...
static Key keys[] = {
	...
	{ MODKEY,                GDK_KEY_g,      scroll,     { .i = 'U' } },
	{ MODKEY|GDK_SHIFT_MASK, GDK_KEY_g,      scroll,     { .i = 'D' } },
...
#define HOMEPAGE "http://localhost:8800"
