...
typedef struct {
	int x, y, w, h;
        XftColor norm[ColLast]; /*xft*/
        XftColor sel[ColLast]; /*xft*/
        Drawable drawable;
	GC gc;
	struct {
		int ascent;
...
static void drawtext(const char *text, XftColor col[ColLast]); /*xft*/
...
static XftColor getcolor(const char *colstr); /*xft*/
...
void
drawtext(const char *text, XftColor col[ColLast]) { /*xft*/
	int i, x, y, h, len, olen;
	char buf[256];
        XftDraw *d; /*xft*/
	XRectangle r = { dc.x, dc.y, dc.w, dc.h };

	XSetForeground(dpy, dc.gc, col[ColBG].pixel); /*xft*/
	...
	if(len < olen) {
        ...
        d = XftDrawCreate(dpy, dc.drawable, DefaultVisual(dpy, screen), DefaultColormap(dpy, screen)); /*xft*/

        XftDrawStringUtf8(d, &col[ColFG], dc.font.xfont, x, y, (XftChar8 *) buf, len);
        XftDrawDestroy(d); /*xft*/
}
...
getcolor(const char *colstr) {
        ...
        if(!XftColorAllocName(dpy, DefaultVisual(dpy, screen), DefaultColormap(dpy, screen), colstr, &color)) /*xft*/
...
setup(void) {
	...
	win = XCreateSimpleWindow(dpy, root, wx, wy, ww, wh, 0,
	                dc.norm[ColFG].pixel, dc.norm[ColBG].pixel); /*xft*/
        XMapRaised(dpy, win);
